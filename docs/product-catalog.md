# Tawaret Hippo Herbals Product Catalog

Last reviewed: 2026-07-13

This is the approved product, size, and price reference. Do not restore discontinued products or use older flyers as a source of truth.

All current products are offered by pre-order. Bank-transfer prepayment secures the order before products are ordered or prepared for sale. Pickup is arranged when the order is ready.

## Current products and offers

| Product | Size or quantity | Price (TTD) | Approved notes |
|---|---:|---:|---|
| Chlorophyll Water | 5 litres | TT$160 | Pre-order model; confirm timing before promising pickup. |
| Chlorophyll Water Bulk Deal | 5 × 5 litres | TT$600 | Five-bottle pre-order option. |
| Magnesium Spray | 250 ml | TT$275 | Use this exact product name. |
| Castor Oil | 1 litre | TT$400 | Confirm availability. |
| Castor Oil | 240 ml | TT$150 | Confirm availability. |
| Colloidal Silver | 25 ppm | TT$400 | Use “Colloidal Silver,” not an invented variation. |
| Healing Oil | Current labelled bottle | TT$275 | Do not rename as “Women’s Healing Oil.” |
| Daily Balance Tonic | 10 × 450 ml bottles | TT$400 | Also called Daily Prebiotic Digestive Tonic when explicitly approved. |
| Daily Balance Tonic | 20 × 450 ml bottles | TT$760 | Confirm flavour availability. |
| Kidney Flush | 2 litres | TT$275 | Standalone product. PDF instructions included. |
| Liver Flush | 2 litres | TT$275 | Standalone product. PDF instructions included. |
| Kidney and Liver Flush Package | Approved package | TT$600 | Includes PDF instructions and a 14-day blood-type meal-prep PDF. |
| Kidney and Liver Flush Combo | 2 × 2 litres | TT$800 | Includes PDF instructions and a 14-day blood-type meal-prep PDF. |
| Colloidal Silver, Healing Oil & Magnesium Bundle | Three-product bundle | TT$800 | Formerly called HSV Protocol. Does not include Kidney Flush or Liver Flush. Use the current name publicly. |
| T.E. Cream | 100 g | TT$275 | Current approved renamed title. |

## Current time-limited preorder

The following prices are valid only through 2026-07-15 and must not remain active after the deadline without renewed approval:

| Product | Preorder price (TTD) | Regular price (TTD) |
|---|---:|---:|
| Chlorophyll Water, 5 litres | TT$125 | TT$160 |
| Chlorophyll Water Bulk Deal, 5 × 5 litres | TT$480 | TT$600 |

- The preorder is secured by prepayment.
- After payment is received, the order is secured for the incoming restock.
- Do not describe ordinary product leads as already being on the chlorophyll waitlist. Add them only after they respond that they want the preorder.

## Digital products and inclusions

- Kidney Flush instructions PDF: planned standalone price TT$150. It must not be advertised as an instant paid download until payment and delivery are technically connected and tested.
- Liver Flush instructions PDF: planned standalone price TT$150. It must not be advertised as an instant paid download until payment and delivery are technically connected and tested.
- Kidney and Liver Flush Package: PDF instructions and 14-day blood-type meal-prep PDF included.
- Kidney and Liver Flush Combo: PDF instructions and 14-day blood-type meal-prep PDF included.
- Do not state that the Colloidal Silver, Healing Oil & Magnesium Bundle includes a meal-prep guide unless Nyla explicitly confirms that inclusion.

## Removed or prohibited legacy listings

Do not restore these as active products or inclusions without explicit approval:

- Womb Detox packages or tea
- Liver & Colon Cleanse capsules
- digestive guide
- vitamins included
- Women’s Healing Oil
- moonditas
- enzyme capsules as a standalone product
- HSV Protocol as the public bundle name
- Tumor Eliminator as the public cream name

## Catalog change rule

When Nyla approves a product, price, size, bundle, or inclusion change:

1. Update this catalog first.
2. Identify every affected website, sales-assistant, form, sheet, message template, and automation reference.
3. Make one structured correction across all approved targets.
4. Test the customer-facing price and WhatsApp journey before publishing.
